;;; lsp-metals-autoloads.el --- automatically extracted autoloads
;;
;;; Code:

(add-to-list 'load-path (directory-file-name
                         (or (file-name-directory #$) (car load-path))))


;;;### (autoloads nil "lsp-metals" "lsp-metals.el" (0 0 0 0))
;;; Generated autoloads from lsp-metals.el

(if (fboundp 'register-definition-prefixes) (register-definition-prefixes "lsp-metals" '("lsp-metals-")))

;;;***

;;;### (autoloads nil "lsp-metals-treeview" "lsp-metals-treeview.el"
;;;;;;  (0 0 0 0))
;;; Generated autoloads from lsp-metals-treeview.el

(if (fboundp 'register-definition-prefixes) (register-definition-prefixes "lsp-metals-treeview" '("lsp-metals-treeview")))

;;;***

;;;### (autoloads nil nil ("lsp-metals-pkg.el" "lsp-metals-protocol.el")
;;;;;;  (0 0 0 0))

;;;***

;; Local Variables:
;; version-control: never
;; no-byte-compile: t
;; no-update-autoloads: t
;; coding: utf-8
;; End:
;;; lsp-metals-autoloads.el ends here
